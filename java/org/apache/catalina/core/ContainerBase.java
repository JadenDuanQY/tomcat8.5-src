...
public abstract class ContainerBase extends LifecycleMBeanBase
    ...
    protected void initInternal() throws LifecycleException {
        ...
        //getStartStopThreadsInternal()默认为1，初始线程池为1，最大为1
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        //也是生命周期类型 继承LifecycleMBeanBase(注册MBean)->LifecycleBase(状态变更)
        ...
        //Future模式 
        /*
         *	1. StartChild实现了Callable<Void>接口
         *	2. submit使用Future<Void>的子类FutureTask<T>的单参构造函数（参数为Callable）初始化并返回Future<Void>实例
        ...
        if (pipeline instanceof Lifecycle) {
        	//这个虽然是Lifecycle类型，但是没有继承LifecycleMBeanBase 而是直接继承了LifecycleBase
        	//这说明没有被注册到jmx中
        	//正常的生命周期的流程，如果是new就执行init init会调用initinternal，然后start会调用startinternal
        	//该方法还会将Valve链全部start，valve是从lifecycleMBeanBase继承的，所以会注册到jmx

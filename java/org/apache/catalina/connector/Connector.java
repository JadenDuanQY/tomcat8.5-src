...
public class Connector extends LifecycleMBeanBase  {
    ...
    public Connector(String protocol) {
        //初始化时 protocol 为null protocolHandlerClassName则设置为org.apache.coyote.http11.Http11NioProtocol
        //*注 catalina通过Digester配置的ConnectorCreateRule规则中调用该构造方法来初始化connector
    ...
    public void setParseBodyMethods(String methods) {
        ...
        //验证是否添加成功
        Boolean t = setProperty("parseBodyMethods", methods);
        int a = 0;
    }
    ...
    protected void initInternal() throws LifecycleException {
        ...
        //server.xml配置protocol属性 通过 set注入set方法会根据protocol设置protocolHandler
        protocolHandler.setAdapter(adapter);

        // Make sure parseBodyMethodsSet has a default
        if (null == parseBodyMethodsSet) {
            //设置需要解析body的方法集，protocolHandler也会被设置，但是会失败，因为设置的属性不存在于protocolHandler
        ...
        //isAprAvailable方法通过查看java lib路径 如果有apr包则返回apr可用, openssl默认开启

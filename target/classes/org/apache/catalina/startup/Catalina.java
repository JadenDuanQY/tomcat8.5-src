...
public class Catalina {
    ...
    public Catalina() {
        //调用一个工具类SecurityConfig 初始化该类的singleton 单例对象，并设置访问包与对应的定义名
     ...
     */
    //digester根据配置通过反射调用配置中的方法来实例化对象
    //其中connecter就是通过配置规则来调用connector的单参数构造方法来进行实例化
    protected Digester createStartDigester() {
        ...
        //为第一个参数对应的setnext添加规则
        digester.addRule("Server/Service/Connector",
                         new ConnectorCreateRule());
        digester.addRule("Server/Service/Connector",
                         new SetAllPropertiesRule(new String[]{"executor", "sslImplementationName"}));
        //addconnector会启动connector
    ...
    public void load() {
        ...
        try {
            ...
            try {
                ...
                //使用digester配置，catalina的server属性被实例化根据server.xml实例化
                digester.push(this);
    ...
    public void start() {
        ...
        if (getServer() == null) {
            //从server.xml加载server属性实例 并初始化server server初始化service service初始化engine与connector
            load();
        }
